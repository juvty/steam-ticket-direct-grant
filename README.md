...
| 5. Steam Web API key | **juvty-payments** (not Keycloak) | Get a Steam Web API key (Steamworks partner site). Set **`STEAM_WEB_API_KEY`** in juvty-payments env (or secret manager). Payments uses it to call Steam (AuthenticateUserTicket, GetOwnedGames). |
